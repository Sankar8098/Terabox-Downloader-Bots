...
async def stats_command(m: UpdateNewMessage):
    if m.is_group or m.is_channel:
        return
    check_if = await is_user_on_chat(bot, f"@SK_MoviesOffl", m.peer_id)
    if not check_if:
        return await m.reply(f"Please join @SK_MoviesOffl then send me the link again.")
    check_if = await is_user_on_chat(bot, f"@SK_MoviesOffl", m.peer_id)
    if not check_if:
        return await m.reply(f"Please join @SK_MoviesOffl then send me the link again.")
...
"""
    ...
    await m.reply(
        ...
        buttons=[
            [
                Button.url("Updates", "https://t.me/SK_MoviesOffl"),
                Button.url(
                    "Repo", "https://github.com/TheDextiN/Terabox-Downloader-Bot"
                ),
            ]
...
async def handle_message(m: Message):
    ...
    check_if = await is_user_on_chat(bot, "@SK_MoviesOffl", m.peer_id)
    if not check_if:
        return await m.reply("Please join @SK_MoviesOffl  then send me the link again.")
    check_if = await is_user_on_chat(bot, "@SK_MoviesOffl", m.peer_id)
    if not check_if:
        return await m.reply("Please join @SK_MoviesOffl then send me the link again.")
    is_spam = db.get(m.sender_id)
    if is_spam and m.sender_id not in [5821871362]:
    ...
    if int(data["sizebytes"]) > 524288000 and m.sender_id not in [5821871362]:
        return await hm.edit(
